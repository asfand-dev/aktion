...
const PAGE_KEYWORDS = {
  ...
  "sx.html": "sx style styling prop universal padding margin gap spacing sizing width height color background bg gradient border radius shadow opacity flex grid display direction align justify wrap grow shrink basis columns position sticky absolute fixed zindex top right bottom left inset font fontsize weight textalign textdecoration typography overflow cursor backdrop blur bgimage bgoverlay hover focus active disabled checked group-hover states animate fade zoom slide pulse responsive breakpoint base sm md lg xl tokens theme-safe className class aria data tooltip hidden logical rtl safe-area inset dvh testid test-id data-testid",
...
function buildTopbar() {
  ...
  const brand = el(
    ...
    el("span", {}, "Aktion"),
    el("span", { class: "topbar-version" }, "v0.6.8"),
  );
