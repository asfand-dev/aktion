...
function aktionPlugin(options = {}) {
  ...
  return {
    ...
    configResolved(config) {
      ...
      if (config.root) projectRoot = node_path.resolve(config.root);
...
function compileAktionFile(entryPath, options = {}) {
  const absolute = node_path.resolve(entryPath);
  return compileAktionSource(node_fs.readFileSync(absolute, "utf8"), absolute, options);
}
function compileAktionSource(source, virtualPath, options = {}) {
  const absolute = node_path.resolve(virtualPath);
  const root = options.root === null ? null : node_path.resolve(options.root ?? node_path.dirname(absolute));
...
function isInsideRoot(candidate, root) {
  const normalisedRoot = node_path.resolve(root);
  const normalised = node_path.resolve(candidate);
  if (normalised === normalisedRoot) return true;
  return normalised.startsWith(normalisedRoot.endsWith(node_path.sep) ? normalisedRoot : normalisedRoot + node_path.sep);
}
...
function createNodeResolver(options = {}) {
  ...
  const allowed = root === null ? null : [node_path.resolve(root), ...(options.roots ?? []).map((r) => node_path.resolve(r)), ...aliases.map(([, t]) => t)];
    ...
    resolve(spec, importerPath) {
      try {
        for (const [prefix, target] of aliases) {
          ...
          const base2 = rest === "" ? target : node_path.resolve(target, rest);
          if (!isInsideRoot(base2, target)) return null;
          return complete(base2);
        }
        if (!spec.startsWith(".") && !spec.startsWith("/")) return null;
        const base = node_path.resolve(node_path.dirname(importerPath), spec);
...
function loadAktionConfig(from) {
  ...
  for (; ; ) {
    const candidate = node_path.resolve(dir, "aktion.config.json");
    if (isFile(candidate)) {
      try {
        const raw = JSON.parse(node_fs.readFileSync(candidate, "utf8"));
        const alias = {};
        for (const [prefix, target] of Object.entries(raw.alias ?? {})) {
          alias[prefix] = node_path.resolve(dir, target);
        }
        return {
          configPath: candidate,
          alias,
          roots: (raw.roots ?? []).map((r) => node_path.resolve(dir, r)),
...
exports.compileAktionFile = compileAktionFile;
exports.compileAktionSource = compileAktionSource;
exports.createNodeResolver = createNodeResolver;
exports.default = aktionPlugin;
exports.isAktionId = isAktionId;
exports.isInsideRoot = isInsideRoot;
exports.loadAktionConfig = loadAktionConfig;
exports.mergeResolveOptions = mergeResolveOptions;
